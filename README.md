...
- AC86U 每天 `13:00` 只检查全部正式央视与省卫；只有正式线路确认不合格时，才快速复验对应家庭备用。
- GitHub 在 `02:25` 和 `13:25` 读取家庭报告。只有当前线路在同一轮家庭路径连续失败、备用在切换前再次通过，且报告与当前正式清单完全绑定时，才按频道和旧 URL 精确替换。
...
`config/home-publisher.json` 目前故意保持 `enabled: false`，路由器安装器也默认本地影子模式。完整步骤见 [`docs/ac86u-home-probe.md`](docs/ac86u-home-probe.md)，实施断点见 [`docs/home-first-roadmap.md`](docs/home-first-roadmap.md)。
