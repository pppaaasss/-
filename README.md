...
当前状态入口，更新于 2026-09-11。GitHub 负责收集公开清单文本与发布家庭决定；AC86U 经与 Apple TV 等价的家庭路径检测。香港不再检测、中转或裁决。
...
- 短期试播下载余量改为 1.05（5%），按 2026-09-11 用户指令执行；人工播放反馈继续优先。保留 50 MiB 暂停、58 MiB 恢复、H.264 3 Mbps。CCTV-4K 接受 1080p，优先合格 2160p。

历史备用曾有 105 个地址、42 个频道。本次报告重建出 11 条备用记录，其中 CCTV-7 用于本次临时换源；其余 10 条放在 [人工试播备用台单](https://raw.githubusercontent.com/pppaaasss/-/master/tv-backup.m3u)。这个数字不是路由器实时备用总数，台单也不是新的合格证明。[本次调整与替换记录](docs/home-headroom-2026-09-11.md)。
...
已启用路由器用 [upgrade_from_termux.sh](router/ac86u/upgrade_from_termux.sh) 更新，参数必须是已发布的 40 位提交 SHA。手机一次下载全部依赖并交给路由器后台安装。升级等待当前批次结束，将下载余量配置迁移至 1.05，保留激活、其余配置、队列和永久去重；不依赖某日旧 ACK。后台完成标志为 `BACKGROUND_READY`。
