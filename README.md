...
4. 实际请求 HLS 主清单、720p/1080p 变体清单和一个视频分片，测首开耗时与分片下载速度后选最快的一条；主清单优先 1080p/720p、HTTPS、CDN 和中文频道。
...
`build-report.txt` 会记录每次构建的候选数、实测通过数、地区限制数、主清单分组数量，以及这 5 个指定 CCTV 台各自的候选数、可播数、清晰度和速度。需要注意：测速发生在 GitHub Actions 线路，最终体验仍会受 Apple TV 所用代理出口和晚高峰影响。
