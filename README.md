...
7. 最后按 `best-fan/iptv-sources` 状态清单覆盖所有同名的 1080p/2160p 频道。这个覆盖层采用用户实际播放线路的结论，不再让 GitHub 海外机房的测速把国内高码 IPTV 源换成低码海外转码；只替换 URL，频道数量、名称、分组、台标和顺序均保持不变。上游只有 720p/576p 的频道不会被强行降级。
