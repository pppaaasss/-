...
若旧文件尚未清理，需要恢复旧测速计划时，先关闭云端派发，并确认旧 Python 故障已修复和旧版负荷可接受，再运行手机入口的 `rollback`。它取得同一组锁，恢复备份中的旧配置、IPTV 启动块及 cron；其他启动服务的后续修改保留，所有旧历史及新断点保留。执行旧文件清理后，须先从手机完整备份恢复并审查，不能直接使用 `rollback`。若只是需要省资源，保持暂停即可。

当前实机验收、清理结果和云端进度字段见 [2026-09-14 现场交接](ac86u-closeout-2026-09-14.md)。
