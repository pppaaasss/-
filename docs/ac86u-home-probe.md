...
> 下方安装说明用于部署参考；本户已安装，不要重复格式化或重装。probe_id 保持 `home-ac86u-8f8908f0fba9`。计划任务入口虽已登记，路径未确认前仍拒绝正式探测；当前不能理解为全台巡检已自动运行。
...
8. 用户验收影子结果后，取得一份家庭基线正常、成功进入 GitHub 且仍新鲜的报告后，即可正式激活。2026-09-07 按用户要求取消额外的四份报告/18 小时观察等待，仍核对实际回传内容哈希：
